...
- **Variabele uitgaven**: vrije lijst per maand (start elke maand leeg), met datum, omschrijving, bedrag en betaald-vinkje. Via het 🧾-icoontje op een post kan je die uitsplitsen in meerdere categorieën (bv. één Visa-afrekening die eigenlijk boodschappen + kleding + overig dekt) — het bedrag van de post zelf blijft tellen voor je budget/totalen, maar de statistieken (verdeling per categorie, trend, export) gebruiken dan de uitsplitsing in plaats van de ene hoofdcategorie.
